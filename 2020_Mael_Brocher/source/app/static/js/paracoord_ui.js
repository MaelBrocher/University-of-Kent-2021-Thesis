...
var ParacoordUI = function (config, options) {
    ...
    function initHeatmapExplorer() {
        ...
        exp.btnSemanticHeatmapFr.on('click', function () {
            getSemanticHeatmap('fr')
            updateHeatmapCore();
        });
        exp.btnSemanticHeatmapEn.on('click', function () {
    ...
    var runPyScriptSemanticHeatmap = function (input, lang, dict, req) {
        var dataS = input +"2431ecfe25e234d51b22ff47701d0fae"+req +"2431ecfe25e234d51b22ff47701d0fae" + lang;
        $.ajax({
            crossDomain: true,
            type: "POST",
    ...
    var getSemanticHeatmap = function (lang) {
        var selected = getSelected();
        var names = "Semantic not ready for :"
        var dict = {}
        for (var i = 0; i < selected.length; i++) {
    ...
    var uploadWords = function (name, words, hmui) {
        ...
        if (hmui.getIsSemantic() == false) 
            runPyScriptUploadHeatmap(words, name, hmui);
    }
